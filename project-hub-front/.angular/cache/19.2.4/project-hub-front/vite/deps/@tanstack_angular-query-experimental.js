...
var QueryClient = class {
  ...
  resetQueries(filters, options) {
    ...
    return notifyManager.batch(() => {
      ...
      return this.refetchQueries(__spreadValues({
        type: "active"
      }, filters), options);
    });
  }
  ...
  invalidateQueries(filters, options = {}) {
    return notifyManager.batch(() => {
      ...
      return this.refetchQueries(__spreadProps(__spreadValues({}, filters), {
...
function hydrate(client, dehydratedState, options) {
  ...
  mutations.forEach((_a) => {
    var _b = _a, {
      state
    } = _b, mutationOptions2 = __objRest(_b, [
      "state"
    ]);
    mutationCache.build(client, __spreadValues(__spreadValues(__spreadValues({}, client.getDefaultOptions().hydrate?.mutations), options?.defaultOptions?.mutations), mutationOptions2), state);
  });
  queries.forEach(({
    queryKey,
    state,
...
function streamedQuery({
  ...
  return (context) => __async(this, null, function* () {
    ...
    const stream = yield queryFn(context);
    try {
...
// node_modules/@tanstack/angular-query-experimental/build/index.mjs
...
function injectInfiniteQuery(injectInfiniteQueryFn, options) {
  return assertInjector(injectInfiniteQuery, options?.injector, () => createBaseQuery(injectInfiniteQueryFn, InfiniteQueryObserver));
}
...
function injectQueries(_a, injector) {
  ...
  return assertInjector(injectQueries, injector, () => {
    ...
    const unsubscribe = ngZone.runOutsideAngular(() => observer.subscribe(notifyManager.batchCalls(result.set)));
    destroyRef.onDestroy(unsubscribe);
    return result;
  });
}
function injectQuery(injectQueryFn, options) {
  return assertInjector(injectQuery, options?.injector, () => createBaseQuery(injectQueryFn, QueryObserver));
}
...
function withDevtools(withDevtoolsFn) {
  ...
  if (!isDevMode() && !withDevtoolsFn) {
    ...
    providers = [{
      ...
      multi: true,
      useFactory: () => {
        if (!isPlatformBrowser(inject(PLATFORM_ID))) return noop2;
        ...
        return () => effect(() => {
          ...
          import("./dev-KB3FVDMW.js").then((queryDevtools) => {
