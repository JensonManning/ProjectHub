...
export class AppKanbanService {

    private _lists: DummyKanbanListType[] = [];

    private selectedCard = new Subject<DummyKanbanCardType>();

    private selectedListId = new Subject<string>();

    private lists = new BehaviorSubject<DummyKanbanListType[]>(this._lists);
    ...
    constructor(private http: HttpClient) {
        this.http
            .get<any>('/demo/data/ph-kanban.json')
            .toPromise()
            .then((res) => res.data as DummyKanbanListType[])
            .then((data) => {
    ...
    addCard(listId: string) {
        ...
        const newCard = { id: cardId, title: title, comments: [], taskList: { title: 'Untitled Task List', tasks: [] } };
    ...
    updateCard(card: DummyKanbanCardType, listId: string) {
    ...
    copyList(list: DummyKanbanListType) {
    ...
    copyCard(card: DummyKanbanCardType, listId: string) {
        let lists = [];
    ...
    moveCard(card: DummyKanbanCardType, targetListId: string, sourceListId: string) {
        if (card.id) {
    ...
    onCardSelect(card: DummyKanbanCardType, listId: string) {
